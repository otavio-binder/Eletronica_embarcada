...
void Trigger_and_Echo(){
    ...
    LATAbits.LATA4 = 0;
    while(!LATAbits.LATA3);
    TMR1_WriteTimer(0);
    TMR1_StartTimer();
    while(LATAbits.LATA3);
    TMR1_StopTimer();
    uint16_t tempo_voo = TMR1_ReadTimer();
    distance = (float)(tempo_voo*340.0/2.0);
...
void main(void)
{
    ...
    TMR0_SetInterruptHandler(send_data);
    while (1)
    {
        if(LATAbits.LATA1 == 0) air_flux += 10;
        if(LATAbits.LATA2 == 0) air_flux -= 10;
        if(air_flux >= 1023) air_flux = 1023;
        if (0 >= air_flux ) air_flux = 0;
