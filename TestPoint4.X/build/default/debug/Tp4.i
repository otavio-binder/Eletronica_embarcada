...
iniciar:
    ...
    banksel OPTION_REG
    movlw 0b00000101 ; Prescaler de 1:256 para Timer0 (ja estamos no banco 1)
    movwf OPTION_REG
    banksel TMR0
    clrf PORTA
...
led_loop:
 ...
 delay_500ms:
    movlw 77 ; 77 ciclos para aproximadamente 500ms
    movwf counter_125ms ; armazenar em counter_125ms

delay_loop:
    movlw 60 ; Valor inicial do TMR0 para overflow em aproximadamente 4ms
    clrwdt
    movwf TMR0 ; carregar o valor inicial
    bcf INTCON, 2 ; limpar a flag do TMR0 (((INTCON) and 07Fh), 2)

wait_for_overflow:
    btfss INTCON, 2 ; verificar a flag do TMR0 (((INTCON) and 07Fh), 2)
    goto wait_for_overflow ; esperar o overflow
    clrwdt
    decfsz counter_125ms, f ; decrementar counter_125ms
...
low_power:
    ...
    movlw 0b00000101 ; setando prescaler para 1:256
    movwf OPTION_REG
    return
