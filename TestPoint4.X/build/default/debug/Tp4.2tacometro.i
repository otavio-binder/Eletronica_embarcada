...
PSECT udata_shr
 counter_125ms: DS 1
 total_revolutions: DS 1
 desired_revolutions: DS 1
 current_revolutions: DS 1

PSECT udata
 seven_seg_digits: DS 2
...
iniciar:
    ...
    BANKSEL TRISC
    clrf TRISC ; Porta C como saída para o display de 7 segmentos

    BANKSEL TRISB
    bcf TRISB, 1 ; Porta B1 como saída para controlar o motor
    bcf TRISB, 4 ; Porta B4 como saída para o anodo do primeiro dígito
    bcf TRISB, 5 ; Porta B5 como saída para o anodo do segundo dígito

    BANKSEL PORTC
    clrf PORTC ; Limpar a porta C

    BANKSEL PORTB
    clrf PORTB ; Limpar a porta B
    bcf PORTB, 1 ; Motor desligado inicialmente

    banksel OPTION_REG
    movlw 0b00000111 ; Prescaler de 1:256 para Timer0
    movwf OPTION_REG

    banksel TMR0
    clrf TMR0 ; Zerando o contador

    bsf INTCON, 5 ; Habilitando a interrupção do TMR0
    bsf INTCON, 7 ; Habilitando as interrupções globais

    movlw 0xFE ; Definir o valor desejado de voltas (254)
    movwf desired_revolutions

main_loop:
    clrwdt ; Limpando o Watchdog Timer

    banksel current_revolutions
    movf current_revolutions, W
    subwf desired_revolutions, W
    btfss STATUS, 2 ; Se current_revolutions == desired_revolutions
    goto motor_on

motor_off:
    BANKSEL PORTB
    bcf PORTB, 1 ; Desligar o motor
    goto main_loop

motor_on:
    BANKSEL PORTB
    bsf PORTB, 1 ; Ligar o motor
    goto main_loop
...
ORG 0x004
    ...
    bcf INTCON, 2 ; Limpando a flag do TMR0

    banksel current_revolutions
    incf current_revolutions, F

    banksel total_revolutions
    incf total_revolutions, F
...
display_update:
    ; Atualizar o display de 7 segmentos com a contagem atual
    banksel current_revolutions
    movf current_revolutions, W
    call hex_to_7seg

    ; Atualizar os dois dígitos do display
    banksel seven_seg_digits
    movf seven_seg_digits, W
    call display_digit1
    movf seven_seg_digits + 1, W
    call display_digit2
    return

display_digit1:
    ; Exibe o primeiro dígito (mais significativo)
    banksel PORTB
    bsf PORTB, 4 ; Ativar o anodo do primeiro dígito
    bcf PORTB, 5 ; Desativar o anodo do segundo dígito
    banksel PORTC
    movwf PORTC
    call delay_5ms ; Pequeno atraso para exibição estável
    return

display_digit2:
    ; Exibe o segundo dígito (menos significativo)
    banksel PORTB
    bcf PORTB, 4 ; Desativar o anodo do primeiro dígito
    bsf PORTB, 5 ; Ativar o anodo do segundo dígito
    banksel PORTC
    movwf PORTC
    call delay_5ms ; Pequeno atraso para exibição estável
    return
...
delay_5ms:
    ; Um pequeno atraso para exibição estável
    movlw 0xA0 ; Aproximadamente 5ms de atraso (ajustar conforme necessário)
    movwf counter_125ms
delay_5ms_loop:
    decfsz counter_125ms, f
    goto delay_5ms_loop
    return

hex_to_7seg:
    ; Converte o valor hexadecimal em dígitos de 7 segmentos
    ; O valor atual está em W
    ; O resultado será armazenado em seven_seg_digits
    banksel seven_seg_digits
    andlw 0x0F
    call hex_to_7seg_digit
    movwf seven_seg_digits

    banksel seven_seg_digits + 1
    andlw 0x0F
    call hex_to_7seg_digit
    movwf seven_seg_digits + 1
    return

hex_to_7seg_digit:
    ; Converte um dígito hexade
addwf PCL, F
    retlw 0b11000000 ; 0
    retlw 0b11111001 ; 1
    retlw 0b10100100 ; 2
