...
const char debug[] = "Armazenando no buffer";
const char debug1[] = "Terminou de armazenar no buffer";
const char debug2[] = "Armazenando na EEPROM";
...
void Agend(){
     if(ang_count - atend_count == 10){
            send_Msg(msg7);
        }else{
            send_Msg(msg4);
            receive = EUSART_Read();
            if(receive == 0x20 || receive == 0x0D)send_Msg(msg5);
            else{
                for(uint8_t i = 0; i<=20 && receive != 0x0D; i++){
                    buffer_rx[i] = receive;
                    buffer_rx[i+1] = 0;
                    receive = EUSART_Read();
                }
...
# 179 "main.c"
    while (1)
    {
        if(EUSART_is_rx_ready()){
            command = EUSART_Read();
            if(command == 0x4C) print_list();
            else if(command == 0x41) Agend();
            else if(command == 0x50) Next();
            else if(command == 0x52) Erase();
            else send_Msg(msg0);
        }
    }
}
